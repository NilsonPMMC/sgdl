...
const loadUsuarios = async () => {
    loading.value = true;
    try {
        const { data } = await ApiService.listarGestaoUsuariosGestor();
        usuarios.value = Array.isArray(data) ? data : data?.results || [];
    } catch (error) {
        usuarios.value = [];
        toast.add({ severity: 'error', summary: 'Gestores', detail: extrairErro(error), life: 4000 });
    } finally {
        loading.value = false;
    }
};
...
const salvar = async () => {
    ...
    try {
        ...
        if (editando.value) {
            ...
            await ApiService.atualizarGestaoUsuarioGestor(editando.value.id, payload);
            toast.add({ severity: 'success', summary: 'Gestor atualizado', life: 2500 });
        } else {
            payload.username = form.value.username;
            payload.password = form.value.password;
            await ApiService.criarGestaoUsuarioGestor(payload);
            toast.add({ severity: 'success', summary: 'Gestor criado', life: 2500 });
        }
...
const tituloDialog = computed(() =>
    editando.value ? `Editar — ${editando.value.username}` : 'Novo gestor'
);
...
<template>
    <div class="flex flex-col gap-6">
        <div class="flex flex-wrap justify-between items-start gap-3">
            <div>
                <h2 class="text-2xl font-semibold m-0">Usuários gestor</h2>
                <p class="text-surface-600 mt-1 mb-0 text-sm">
                    Administradores do sistema com acesso pleno ao SGDL e ao Django Admin
                    (<code>is_staff</code> + <code>is_superuser</code>). Órgão e setor são
                    referência institucional opcional — não limitam o escopo.
                </p>
            </div>
            <Button label="Novo gestor" icon="pi pi-user-plus" @click="abrirNovo" />
        </div>

        <Message severity="info" :closable="false" class="text-sm m-0">
            Gestores acessam todas as secretarias, configurações e o painel
            <a href="/admin/" target="_blank" rel="noopener" class="text-primary">/admin/</a>.
        </Message>
            ...
            <template #content>
                ...
                >
                    ...
                    <Column header="Admin">
                        <template #body="{ data }">
                            <Tag
                                :value="data.vinculo_gestor?.admin_pleno ? 'Pleno' : 'Incompleto'"
                                :severity="data.vinculo_gestor?.admin_pleno ? 'success' : 'danger'"
                            />
                        </template>
                    </Column>
                    <Column field="secretaria_nome" header="Ref. órgão" />
                    <Column header="Ref. setor(es)">
                        <template #body="{ data }">
        ...
        <Dialog v-model:visible="dialogAberto" :header="tituloDialog" modal class="w-full max-w-2xl">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium">Órgão de referência (opcional)</label>
                    <Select
                        v-model="form.sinapse_orgao_id"
                        :options="orgaos"
                        optionLabel="label"
                        optionValue="value"
                        placeholder="Opcional — ex.: SMGOV"
                        filter
                        showClear
                        class="w-full"
                    />
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium">Setor(es) de referência (opcional)</label>
                    <MultiSelect
                        v-model="form.unidade_ids"
                        :options="setoresOrgao"
                        optionLabel="label"
                        optionValue="value"
                        placeholder="Opcional — metadado institucional"
                        filter
                        display="chip"
                        class="w-full"
