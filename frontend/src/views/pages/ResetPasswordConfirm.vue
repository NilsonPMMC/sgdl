                    ...
                    <form @submit.prevent="handleResetConfirm">
                        <div class="flex flex-col gap-4">
                            <div>
                                ...
                                <Password id="password" v-model="password" placeholder="Digite a nova senha" :feedback="true" toggleMask fluid />
                            </div>
                            <div>
                                <label for="passwordConfirm" class="block mb-2">Confirmar Nova Senha</label>
                                <Password id="passwordConfirm" v-model="passwordConfirm" placeholder="Confirme a nova senha" :feedback="false" toggleMask fluid :invalid="!!error" />
                            </div>
                        </div>

                        <Message v-if="error" severity="error" :closable="false" class="mt-4">{{ error }}</Message>

                        <Button label="Salvar Nova Senha" class="w-full p-3 text-xl mt-5" type="submit" :loading="loading" />
                    </form>
                </div>
