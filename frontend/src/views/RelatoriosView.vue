...
import { useUserStore } from '@/stores/userStore'; // <-- 1. IMPORTADO
...
const userStore = useUserStore(); // <-- 2. INSTANCIADO
...
// <-- 3. FUNÇÃO formatarParams() ATUALIZADA ---
const formatarParams = () => {
    const params = {};
    const currentUser = userStore.currentUser; // Pega o usuário logado
    ...
    // --- Lógica de Filtro por Perfil (SECRETARIA) ---
    if (currentUser && currentUser.perfil === 'SECRETARIA') {
        // CORREÇÃO: Converter o ID numérico para String
        params.secretaria__in = String(currentUser.secretaria);
    } else if (filtros.secretarias && filtros.secretarias.length > 0) {
    ...
    // --- Lógica de Filtro por Perfil (VEREADOR) ---
    if (currentUser && currentUser.perfil === 'VEREADOR') {
        // CORREÇÃO: Converter o ID numérico para String
        params.vereador__in = String(currentUser.id);
    } else if (filtros.vereadores && filtros.vereadores.length > 0) {
...
const carregarOpcoesFiltros = async () => {
    try {
        const [respSecretarias, respServicos, respVereadores] = await Promise.all([apiService.getSecretarias(), apiService.getServicos(), apiService.getUsuarios({ perfil: 'VEREADOR' })]);
...
const buscarRelatorios = async () => {
    ...
    try {
        const [respKPIs, respStatus, respSecretaria, respDemandas] = await Promise.all([apiService.getReportKPIs(params), apiService.getReportPorStatus(params), apiService.getReportPorSecretaria(params), apiService.getReportDemandasList(params)]);
...
const limparFiltros = () => {
    ...
    filtros.vereadores = null; // Esta linha será mantida, mas 'formatarParams' irá sobrepô-la se necessário
    buscarRelatorios();
};
...
const imprimirPagina = async () => {
    ...
    await nextTick(); // 3. Chama a impressão
...
onMounted(() => {
    ...
    buscarRelatorios(); // Só precisamos escutar o 'afterprint'
    window.addEventListener('afterprint', afterPrint);
});

onUnmounted(() => {
    // 3. Remove os listeners ao sair da página
    window.removeEventListener('afterprint', afterPrint); // Corrigido 'beforePrint' para 'afterprint'
});
...
<template>
    ...
    <div v-if="!isLoading && isLoaded" class="relatorio-container">
        ...
        <div class="grid grid-cols-12 gap-8 print-avoid-break print-chart-grid">
            <div v-if="userStore.currentUser?.perfil !== 'SECRETARIA'" class="col-span-12 lg:col-span-6 xl:col-span-6">
                <div class="card">
